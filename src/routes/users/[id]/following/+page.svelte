<script lang="ts">
    ...
    let tabSet: number = 0;
    let users: User[] = data.following.users;
    let cursor: string | null = data.following.next;
    ...
    const infiniteHandler = async ({ detail: { loaded, complete } }: InfiniteEvent) => {
        ...
        const url = `/users/${data.user.id}/profile/following?cursor=${cursor}`;
...
<AtlasMetaTags title={`${user.username}'s Followings`} description={`Check out who ${user.username} follows.`} image={picture(user.picture, 'medium')} />
...
<Container>
    ...
    <div class="card p-4">
        <div class="flex flex-row gap-4">
            <div class="flex-grow">
                <div class="text-3xl">Followed By {user.username}</div>
            </div>
        </div>
    </div>
    <br />
